    ...
    const filteredProducts = products.filter((product) =>
      ...
      if (!category || !products) return <div>Loading...</div>;
       ...
       return (
        ...
        <h1 className=
        "text-4xl font-bold text-center text-gray-800 mb-8">
          {category.name} 
        </h1>
  
        {/* Search input */}
        <div className=
        "mb-6 flex items-center justify-center space-x-2">
          ...
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product) => (
              ...
              >
                ...
                <h3 className=
                "text-xl font-semibold text-gray-800">{product.title}</h3>
                <p className=
                "text-xl font-bold text-green-600 mt-4">${product.price}</p>
              </div>
            ))
          ) : (
